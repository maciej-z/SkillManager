...
- **Supabase is optional at runtime**: `SUPABASE_URL`/`SUPABASE_KEY` are declared `optional: true` in the env schema. `createClient()` returns `null` when either is unset, and `src/middleware.ts` treats that as a logged-out user rather than throwing. `src/lib/config-status.ts` tracks this as `missingConfigs`, surfaced as a warning banner by `Layout.astro`. Any new code touching the Supabase client must handle the `null` case.
- Auth forms (`SignInForm.tsx`, `SignUpForm.tsx`) are native `<form method="POST" action="/api/auth/...">` submissions, not fetch/JSON calls — client-side validation only calls `preventDefault()` to block a submit; the actual auth logic and redirects happen server-side in the API route.
...
GitHub Actions workflow (`.github/workflows/ci.yml`) runs `npx astro sync` (regenerates `.astro/types.d.ts`), then lint, then build on every push and PR to master. Requires `SUPABASE_URL` and `SUPABASE_KEY` repository secrets for the build step.
